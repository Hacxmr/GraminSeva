  ...
  const testInitiateCallApi = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        addLog(`⚠️ POST /api/initiate-call response: ${data.error || "Check Twilio credentials"}`)
        // Mark as success if it's a Twilio configuration issue (not a code issue)
        if (data.error && data.error.includes("Twilio")) {
          addLog(`ℹ️ API endpoint is working. Configure Twilio credentials in .env file.`)
          setResults((prev) => ({ ...prev, initiateCallApi: "success" }))
        } else {
